...
public class TokenLemmaMappingAnalyzer {
	...
	/**
	 * This class is used to find out how simliar the emotions of tokens are
	 * whiche will be mapped on the same lemma. Therefore, it provides data on
	 * the number of tokens per lemma and univariant statistics for them.
	 * 
	 * @param args
	 */
	static public void main(String[] args) throws Exception {
		...
		while ((line = bReader.readLine()) != null) {
			// the lines at the beginning of the file starting with // are
			// comments. Therefore, should not be read to EmotionVector.
			while (line.startsWith("//")) {
				line = bReader.readLine();
			}
			String[] line2Array = line.split("\t");
			String currentWord = line2Array[0];
			if (line2Array.length == 4) { // checks if csv-entry is well-formed.
											// Otherwise, OutOfArray-Exepction
											// may occur.
				// transforms the last 3 coloums of the csv-file into an emotion
				// Vector. String values have to be transformed to double using
				// Double.parseDouble(string).#
				// reads EmotionVector of token currentWord
				EmotionVector currentVector = new EmotionVector(
						Double.parseDouble(line2Array[1]),
						Double.parseDouble(line2Array[2]),
						Double.parseDouble(line2Array[3]));
				// filing tokenList with all the entries from the external
				// Lexicon
		...
		for (Token currentToken : tokenList) {
			...
			String lemmatizedToken = lemmatizer
					.lemmatizeToken(currentToken.tokenName);
		...
		System.setOut(new PrintStream(new File(
				"Token_Lemma_Mapping_2015-12-05.csv")));
